...
output: 
    p-q-frequ.json
        the property -> (qualifier -> frequeny) dictionary
        for each property p, for each qualifier q, the number of statements with property p (main snack p)
        that are qualified by q
        on each line : <property> : {<qualifier> : <frequency>, ...}
    p-freq.json
        on each line : <property>: <frequency> (# statements with this property (main snack p))
    q-freq.json
        on each line : <qualifier>: <frequency> (# statements with this qualifier)
